...
func GetLeaseDetails() (res []contradbschema.LeaseDetailsByIPHostname, err error) {
	...
	rows, err = xdb.Queryx(`SELECT time, ip, mac, hostname, vendor FROM contracore.lease_details_by_ip_hostname;`)
	if err != nil {
	...
	for rows.Next() {
		...
		res = append(res, contradbschema.LeaseDetailsByIPHostname{
			Time:     n.Time,
			IP:       n.IP.IPNet.IP,
			MAC:      n.MAC.Addr,
			Hostname: n.Hostname,
...
func GetOUI() (res []contradbschema.OUI, err error) {
	...
	err = xdb.Select(&res, `SELECT * FROM contracore.oui;`)
...
func GetConfig() (res contradbschema.Config, err error) {
	...
	err = xdb.Get(&res, `SELECT * FROM contracore.config ORDER BY id DESC LIMIT 1`)
	return res, errOfflineOrOriginal(err)
}
...
func GetBlacklistRules() (res []contradbschema.Blacklist, err error) {
	...
	err = xdb.Select(&res, `SELECT * FROM contracore.blacklist;`)
	return res, errOfflineOrOriginal(err)
}
...
func GetWhitelistRules() (res []contradbschema.Whitelist, err error) {
	...
	rows, err = xdb.Queryx(`SELECT * FROM contracore.whitelist;`)
	if err != nil {
		return nil, errOfflineOrOriginal(err)
	}
