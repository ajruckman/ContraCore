...
func main() {
    ...
    w.WriteString("import `github.com/ajruckman/ContraCore/internal/state`\n\n")
    w.WriteString("func Setup() {\n")
    w.WriteString("    clog := log.NewWithPlugin(`contradomain`)\n")
    w.WriteString("    if !state.PostgresOnline.Load() {\n")
    ...
    w.WriteString("    _, err := contradb.PDB.Exec(context.Background(), `\n")
